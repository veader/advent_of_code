...
of adding two snailfish numbers can result in snailfish numbers that need to be reduced.
...
of the list of actions. For example, if **split** produces a pair that meets the 
...
left of the exploding pair (_if any_), and the pair's right value is added to the first 
regular number to the right of the exploding pair (_if any_). Exploding pairs will always 
...
- after addition: `[[[[[4,3],4],4],[7,[[8,4],9]]],[1,1]]` -: first `[4,3]` to explode
- after explode:  `[[[[0,7],4],[7,[[8,4],9]]],[1,1]]`     -: middle `[8,4]` to explode
- after explode:  `[[[[0,7],4],[15,[0,13]]],[1,1]]`       -: middle `15` to split
- after split:    `[[[[0,7],4],[[7,8],[0,13]]],[1,1]]`    -: right `13` to split
- after split:    `[[[[0,7],4],[[7,8],[0,[6,7]]]],[1,1]]` -: right `[6,7]` to explode
...
The homework assignment involves adding up a **list of snailfish numbers** (_your puzzle 
input_). The snailfish numbers are each listed on a separate line. Add the first 
snailfish number and the second, then add that result and the third, then add that result 
and the fourth, and so on until all numbers in the list have been used once.
