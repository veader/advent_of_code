...
class AntennaMap {
    ...
    func findAntinodes() -> [Coordinate] {
        antennas.keys.flatMap { findAntinodes(for: $0) }.unique()
    }

    func findAntinodes(for freq: String) -> [Coordinate] {
        var nodes: [Coordinate] = []

        let ants = antennas[freq] ?? []
        for pair in ants.pairCombinations() {
            nodes.append(contentsOf: antinodes(for: pair[0], and: pair[1]))
        }

        return nodes
    }

    func antinodes(for c1: Coordinate, and c2: Coordinate) -> [Coordinate] {
        let points = [c1, c2].sorted()
        let delta = points[0].delta(to: points[1])
        let nodes = [points[0].applying(delta: delta, originTopLeft: true),
                     points[1].applying(delta: delta.opposite, originTopLeft: true)]
