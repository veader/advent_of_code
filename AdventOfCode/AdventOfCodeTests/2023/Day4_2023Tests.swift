...
final class Day4_2023Tests: XCTestCase {
    ...
    func testPart1() async throws {
        let answer = await Day4_2023().run(part: 1, sampleInput)
        XCTAssertEqual(13, answer as? Int)
    }

    func testPart1Answer() async throws {
        let answer = await Day4_2023().run(part: 1)
        XCTAssertEqual(17803, answer as? Int)
    }

    func testPart2() async throws {
        let answer = await Day4_2023().run(part: 2, sampleInput)
        XCTAssertEqual(30, answer as? Int)
    }

    func testPart2Answer() async throws {
        let answer = await Day4_2023().run(part: 2)
